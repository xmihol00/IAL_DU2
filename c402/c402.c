...
**/
	...
	if (tmp == NULL)
	{
		*RootPtr = (tBTNodePtr) malloc(sizeof(struct tBTNode));
		if (*RootPtr == NULL)
		{
			// reseni problemu s nedostatkem pameti
			exit(99);
		}
	...
	while (tmp != NULL)
	{
		...
		else if (tmp->Cont > Content)
		{
			if (tmp->LPtr == NULL)
			{
				tmp->LPtr = (tBTNodePtr) malloc(sizeof(struct tBTNode));
				if (tmp->LPtr == NULL)
				{
					// reseni nedostatku pameti
		...
		else
		{
			if (tmp->RPtr == NULL)
			{
				tmp->RPtr = (tBTNodePtr) malloc(sizeof(struct tBTNode));
				if (tmp->RPtr == NULL)
				{
					// reseni nedostatku pameti
					BTDisposeTree(RootPtr);
