...
**/
	...
	if (*RootPtr == NULL)
	{
		*RootPtr = (tBSTNodePtr)malloc(sizeof(struct tBSTNode));
		if (*RootPtr == NULL)
		{
			// osetreni stavu, kdy nebyla pridelena pamet
			exit(99);
		}
		(*RootPtr)->LPtr = NULL;
		(*RootPtr)->RPtr = NULL;
...
**/
	...
	if ((*RootPtr)->RPtr->RPtr == NULL)
	{
		...
		if ((*RootPtr)->RPtr->LPtr != NULL)
		{
			tBSTNodePtr tmp = (*RootPtr)->RPtr->LPtr;
			(*RootPtr)->RPtr->Key = tmp->Key;
			(*RootPtr)->RPtr->BSTNodeCont = tmp->BSTNodeCont;
			(*RootPtr)->RPtr->RPtr = tmp->RPtr;
			(*RootPtr)->RPtr->LPtr = tmp->LPtr;
			free(tmp);
		}
		else
		{
	...
	else if ((*RootPtr)->Key == K)
	{
		...
		if ((*RootPtr)->RPtr != NULL && (*RootPtr)->LPtr != NULL)
		{
			...
			else
			{
				...
				(*RootPtr)->BSTNodeCont = dispose->BSTNodeCont;
				(*RootPtr)->Key = dispose->Key;
				(*RootPtr)->LPtr = dispose->LPtr;
				free(dispose);
			}
